...
def plot_histogram(d: chex.Array, ax: plt.Axes, *args, **kwargs):
...
def make_default_plotter(
    ...
    def get_data_for_plotting(state: TrainingState, key: chex.PRNGKey, train_data=train_data, test_data=test_data):
        ...
        return pairwise_distances_flow_x, pairwise_distances_flow_a, pairwise_distance_flow_a_minus_x, \
            pairwise_distances_target_a, pairwise_distance_target_a_minus_x


    def default_plotter(state: TrainingState, key: chex.PRNGKey):
        # Plot interatomic distance histograms.
        pairwise_distances_flow_x, pairwise_distances_flow_a, pairwise_distance_flow_a_minus_x, \
            pairwise_distances_target_a, pairwise_distance_target_a_minus_x = get_data_for_plotting(
            state, key
        )

        # Plot original coords
        fig1, axs = plt.subplots(1, 2, figsize=(10, 5))
        plot_histogram(pairwise_distances_flow_x, axs[0], label="flow samples")
        plot_histogram(pairwise_distances_flow_x, axs[1], label="flow samples")
        plot_histogram(pairwise_distances_test_x, axs[0],  label="train samples")
        plot_histogram(pairwise_distances_train_x, axs[1],  label="test samples")
        ...
        for i in range(flow.n_augmented):
            d_a_flow_single = pairwise_distances_flow_a[i]  # get single group of augmented coordinates
            d_a_target = pairwise_distances_target_a[i]  # Get first set of aux variables.
            chex.assert_equal_shape((pairwise_distances_flow_x, d_a_flow_single, d_a_target))
            plot_histogram(d_a_flow_single, axs2[i], label="flow samples")
            plot_histogram(d_a_target, axs2[i], label="test samples")
        ...
        for i in range(flow.n_augmented):
            d_a_minus_x_flow_single = pairwise_distance_flow_a_minus_x[i]  # get single group of augmented coordinates
            d_a_minus_x_target_single = pairwise_distance_target_a_minus_x[i]  # Get first set of aux variables.
            chex.assert_equal_shape((pairwise_distances_flow_x, d_a_minus_x_flow_single, d_a_minus_x_target_single))
            plot_histogram(d_a_minus_x_flow_single, axs3[i], label="flow samples")
            plot_histogram(d_a_minus_x_target_single, axs3[i], label="test samples")

...
from nets.e3nn_transformer import EnTransformerConfig, EnTransformerBlockConfig, EnTransformerBlock, \
    EnTransformer, TransformerTorsoConfig
...
def layer_test():
    ...
    layer_config = EnTransformerBlockConfig(
        num_heads=2, n_vectors_hidden=3, n_invariant_feat_hidden=2, bessel_number=3, r_max=max_radius, mlp_units=(4,))
    ...
    positions = jax.random.normal(subkey, (n_nodes, layer_config.n_vectors_hidden, dim))
    features = jax.random.normal(subkey, (n_nodes, layer_config.n_invariant_feat_hidden))
...
def transformer_test():
    num_heads = 1
    layer_config = EnTransformerBlockConfig(
        num_heads=num_heads, n_vectors_hidden=num_heads*2, n_invariant_feat_hidden=num_heads*3, bessel_number=8, r_max=10.0, mlp_units=(4,))
    torso_config = TransformerTorsoConfig(block_config=layer_config, n_blocks=3, layer_stack=False)

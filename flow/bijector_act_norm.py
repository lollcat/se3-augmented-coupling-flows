...
def global_scaling_and_shift(x, global_scaling, centre_shift_interp, alt_coords_global_mean):
    """
    Global scaling:
...
class GlobalScaling(distrax.Bijector):
    def __init__(self, log_global_scale,
        ...
        if activation == jnp.exp:
            ...
            self._inv_global_scale = jnp.exp(jnp.negative(log_global_scale))

            self._centre_shift_interp = jnp.exp(log_centre_shift_interp)
            self._inv_centre_shift_interp = jnp.exp(jnp.negative(log_centre_shift_interp))
            ...
            self._inv_global_scale = 1. / self._global_scale
            ...
            self._inv_centre_shift_interp = 1. / self._centre_shift_interp
    ...
    def forward(self, x: chex.Array) -> chex.Array:
        """Computes y = f(x)."""
        if len(x.shape) == 2:
            return global_scaling_and_shift(x, self._global_scale, self._centre_shift_interp,
                                            self.alt_coords_global_mean)
        elif len(x.shape) == 3:
            return jax.vmap(global_scaling_and_shift)(x, self._global_scale, self._centre_shift_interp,
                                            self.alt_coords_global_mean)
        else:
            raise Exception
    ...
    def inverse(self, y: chex.Array) -> chex.Array:
        """Computes x = f^{-1}(y)."""
        if len(y.shape) == 2:
            return global_scaling_and_shift(y, self._inv_global_scale, self._inv_centre_shift_interp,
                                            self.alt_coords_global_mean)
        elif len(y.shape) == 3:
            return jax.vmap(global_scaling_and_shift)(y, self._inv_global_scale, self._inv_centre_shift_interp,
                                            self.alt_coords_global_mean)
        else:
            raise Exception
...
def make_conditioner(get_scale_fn, get_shift_fn):
    def conditioner(x):
        ...
        centre_of_mass = jnp.mean(x, axis=-1, keepdims=True)
